...
{
    if ([HWFile isFileExist:filePath]) {
        return YES;
    }
           ...
           isReplace:(BOOL)isReplace
{
    ...
    if ([HWFile isFileExist:toPath] && isReplace) {
        [HWFile deleteFile:toPath];
    }
                   ...
                   isReplace:(BOOL)isReplace
{
    ...
    for (NSString *path in contents) {
        ...
        if ([HWFile isFileExist:toFilePath] && isReplace) {
            [HWFile deleteFile:toFilePath];
        }
...
+ (double)calculteFileSzie:(NSString *)filePath
{
    ...
    if (dirContents == nil) {//文件夹为空时,dirContents不为nil但数目count为0,path为文件时,dirContents=nil
        ...
        for (NSString *dirName in dirContents) {
            fSize +=  [HWFile calculteFileSzie:[filePath stringByAppendingPathComponent:dirName]] ;
        }
    }
    ...
    if (dir != NULL) {
        ...
        while ((child = readdir(dir))!=NULL) {
            ...
            if (child->d_type == DT_DIR) {
                folderSize += [HWFile calculteFileSizeAtPath:childPath];
                // 把目录本身所占的空间也加上
...
+ (double)calculteFileSzieEx:(NSString *)filePath
{
    ...
    return [HWFile calculteFileSizeAtPath:[filePath cStringUsingEncoding:NSUTF8StringEncoding]];
}
...
+ (void)deleteFiles:(NSArray *)fileNames inPath:(NSString *)path
{
    ...
    if (dirContents == nil) {//文件夹为空时,dirContents不为nil但数目count为0,path为文件时,dirContents=nil
        if ([fileNames containsObject:[path lastPathComponent]]) {
            [HWFile deleteFile:path];
        }
    }
    else {
        for (NSString *dirName in dirContents) {
            if ([fileNames containsObject:dirName]) {
                [HWFile deleteFile:[path stringByAppendingPathComponent:dirName]];
            }
            else {
                [HWFile deleteFiles:fileNames inPath: [path stringByAppendingPathComponent:dirName]];
            }
        }
    }
}
