...
@implementation HWBrowserImageView
{
    __weak HWWaitingView *_waitingView;
    BOOL _didCheckSize;
...
- (void)layoutSubviews
{
    ...
    if (self.bounds.size.width * (imageSize.height / imageSize.width) > self.bounds.size.height) {
        if (!_scroll) {
            ...
            scroll.backgroundColor = HWPhotoBrowserBackgrounColor;
            _scroll = scroll;
...
{
    HWWaitingView *waiting = [[HWWaitingView alloc] init];
    waiting.bounds = CGRectMake(0, 0, 100, 100);
    waiting.mode = HWWaitingViewProgressMode;
    _waitingView = waiting;
    [self addSubview:waiting];
    
    
    __weak HWBrowserImageView *imageViewWeak = self;
#warning 这里要加载网络图片
//    UIImage *image;
...
- (void)prepareForImageViewScaling
{
    if (!_zoomingScroolView) {
        ...
        _zoomingScroolView.backgroundColor = HWPhotoBrowserBackgrounColor;
